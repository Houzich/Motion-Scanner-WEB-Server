...
sorter.prototype.pages=function(){this.d=Math.ceil(this.l/this.pagesize)};
 ...
 for (var i = 0; i < rows; i++) {var elem=rowTpl.cloneNode(true);tbl.insertBefore(elem,tbl.children[0]);if(fl_highlight){$(elem).addClass('highlight').delay(1000).queue(function(next){ $(this).removeClass('highlight'); alarm_history_tbl.sort(1); alarm_history_tbl.sort(1); next();});}}
 ...
 var tableRows = tbl.rows;
  ...
  tableRows[0].cells[6].innerHTML = "<a onclick='DelRowAlarmHistory(this)'> Delete </a><input id='this_id' type='hidden'><div class='clear'></div>";
    ...
    //console.info(tbl.children.length);
     //if(tbl.children.length>20){for(i=20;i<tbl.children.length;i++){tbl.removeChild(tbl.children[i])}}
...
*/
     ...
     this.add_row_arr = function (fl_highlight, tbl_arr) {
         ...
         for (i in tbl_arr){
             ...
             $(tbl.children[sort_size.value]).addClass('disp_none');
         ...
         if(tbl.children.length>19){for(i=19;i<tbl.children.length;i++){tbl.removeChild(tbl.children[i])}}
